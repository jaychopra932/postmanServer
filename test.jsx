...
class MainComponent extends Component {
  ...
  submitFunction = () => {
    ...
    if (method == "GET") {
      let response = this.getData(url, 1697460986543);
    } else if (method == "POST") {
  ...
  getData = (url, json) => {
    ...
    // axios
    //   .get(url, { headers: { Authorization: 1697460986543 } })
    //   .then((response) => {
    ...
    async function getFn(url) {
      try {
        let response = await axios.get(url, {
          headers: { Authorization: "Bearer 1697460986543" },
        });
